...
def dnn_init(dataset, sensitive_param, max_iter):
    """
    ...
    model_config = {"LogisticRegression": LogisticRegression(C=1.0, penalty='l2', solver='liblinear', max_iter=max_iter), "SVC": SVC(kernel='rbf', probability=True, max_iter=max_iter),
                    "DecisionTreeClassifier": DecisionTreeClassifier(), "MLPClassifier": MLPClassifier(max_iter=max_iter), "XGBRegressor": XGBRegressor()}
    invert_model_config = {
        ...
        "DecisionTreeClassifier": DecisionTreeClassifier(), "MLPClassifier": MLPClassifier(max_iter=max_iter),
    ...
    model.fit(X, Y)
    invert_X = invert_sensitive(X, FLAGS.sens_param)
    invert_model = invert_model_config[FLAGS.model_config]
    ...
    if FLAGS.shap:
        shap.initjs()
        if FLAGS.model_config == 'LogisticRegression':
            explainer = shap.LinearExplainer(model, X)
            idi = np.array(init_list)
            shap_values = explainer.shap_values(idi)
        elif FLAGS.model_config == 'DecisionTreeClassifier':
            explainer = shap.TreeExplainer(model, X)
            idi = np.array(init_list)
            shap_values = explainer.shap_values(idi)[0]
        elif FLAGS.model_config == 'SVC':
    ...
    np.save('../results/'+dataset+'/'+ str(sensitive_param) + '/cluster_'+FLAGS.model_config+'_init_samples.npy', np.array(idi))
