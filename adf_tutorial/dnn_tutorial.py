...
def dnn_fair_testing(dataset, sensitive_param, model_path, cluster_num, max_global, max_local, max_iter, new_input):
    """
    ...
    if new_input:
        ...
        inputs = init_sample[:min(max_global, len(init_index))]
    else:
    ...
    for num in range(len(inputs)):
        if new_input:
            sample = np.array([init_sample[num]])
            index = init_index[num]
        else:
            index = inputs[num]
        ...
        for iter in range(max_iter+1):
            ...
            ##marvin adds##
            if label == n_label and (tuple(temp) not in global_miss):
    ...
    np.save('../results/'+dataset+'/'+ str(sensitive_param) + '/global_miss.npy', np.array(global_miss_list))
