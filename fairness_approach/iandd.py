...
def check_for_error_condition(model_name, model, conf, t, sens):
    t = t.astype('int')
...
def iandd(dataset, sensitive_param, max_iter, sample_limit, model_name):
    data = {
        "census":census_data,
        "bank":bank_data,
        "compas": compas_data
    ...
    for i in range(len(X)):
        ...
        if y_orig != y_new:
            total_init += 1
            if check_for_error_condition(model_name, model, data_config[dataset], X[i], sensitive_param):
